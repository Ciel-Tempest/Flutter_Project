...
const String colMacroJourneyText = 'Macro Journey'; // Text column (use G for Macro Journey name)
const String colMacroJourneyId = '# Macro Journey'; // ID column (use F for Macro Journey ID)
const String colMicroJourneyText = 'Micro Journey'; // Text column (use I for Micro Journey name)
const String colMicroJourneyId = '# Micro Journey'; // ID column (use H for Micro Journey ID)
const String colSubJourneyText = 'Sub Journey'; // Text column (use K for Sub Journey name)
const String colMicroSubId = 'Micro&Sub'; // Combined ID column (use L for unique Leaf ID)
// --- End Constants ---
...
String _createPathId(String? parentPath, String? currentLabel) {
    ...
    return parentPath == null || parentPath.isEmpty ? currentLabel : '$parentPath > $currentLabel';
}


@riverpod
Future<Graph> fullGraph(FullGraphRef ref) async { // Renamed provider
  ...
  final headerRow = sheet.rows.firstWhere((row) => row.any((cell) => cell?.value != null));
  ...
  for (var i = 1; i < sheet.maxRows; i++) {
    ...
    if (row.every((cell) => cell == null || cell.value == null || cell.value.toString().isEmpty)) {
    ...
    // --- FILTERING ---
    if (customerNeed == null || customerNeed.isEmpty || customerNeed == "New customer journeys to be added - FY25") {
    ...
    final macroJourneyLabel = _cellValue(row, colIndices[colMacroJourneyText] ?? -1);
    final microJourneyLabel = _cellValue(row, colIndices[colMicroJourneyText] ?? -1);
    final subJourneyLabel = _cellValue(row, colIndices[colSubJourneyText] ?? -1);
    ...
    Node getNodeAndAddEdge(String? label, int level, String parentPathId, Node? currentParent) {
        if (label == null || label.isEmpty) {
             if (currentParent == null) throw Exception("Cannot proceed without parent for level $level");
        ...
        if (!allNodes.containsKey(pathId)) {
            ...
            if (currentParent != null) {
                 print("Adding edge: ${(currentParent.key?.value as MindMapNode).label} -> ${newNodeData.label}");
    ...
    try {
      ...
      parentNode = getNodeAndAddEdge(macroJourneyLabel, 4, currentPathId, parentNode);
      currentPathId = _createPathId(currentPathId, macroJourneyLabel);

      // Level 5: Micro Journey
      parentNode = getNodeAndAddEdge(microJourneyLabel, 5, currentPathId, parentNode);
      ...
      if (subJourneyLabel != null && subJourneyLabel.isNotEmpty) {
        parentNode = getNodeAndAddEdge(subJourneyLabel, 6, currentPathId, parentNode);
  ...
  print("Excel processing finished. ${graph.nodeCount} nodes, ${graph.edges.length} edges.");
  if (graph.nodes.where((n)=>(n.key?.value as MindMapNode).level == 1).isEmpty) {
