...
class MindMapScreen extends ConsumerWidget {
  ...
  Color getNodeColor(int level, bool isActive, BuildContext context) {
   ...
   Color getBorderColor(int level, bool isExpanded, bool isActive, BuildContext context) {
      if (isActive) return Colors.orangeAccent[400]!; // Distinct border for active
      ...
      return Colors.white54; // Alternative: Simple white border for inactive collapsed
   }

   // Define text color (ensure contrast with background)
   Color getTextColor(int level, bool isExpanded, bool isActive, BuildContext context) {
      Color bgColor;
      ...
      return ThemeData.estimateBrightnessForColor(bgColor) == Brightness.dark
          ? Colors.white.withOpacity(0.95) // Slightly more opaque white for dark backgrounds
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final algorithm = FruchtermanReingoldAlgorithm(iterations: 300); // Stick with force-directed
    ...
    return Scaffold(
      ...
      body: graphDisplayStateAsync.when(
        ...
        data: (graphDisplayState) { // Data is GraphDisplayState
          final graphToDisplay = graphDisplayState.displayedGraph;
          final expandedNodeIds = graphDisplayState.expandedNodeIds;
          final activeNodeId = graphDisplayState.lastTappedNodeId; // <<< Get the active ID
          ...
          return InteractiveViewer(
            ...
            child: GraphView(
              ...
              builder: (Node node) {
                ...
                bool isActive = nodeData.id == activeNodeId; // <<< Check if this node is active

                // --- Get Dynamic Colors ---
                Color nodeColor = getNodeColor(nodeData.level, isActive, context);
                Color borderColor = getBorderColor(nodeData.level, isExpanded, isActive, context);
                Color textColor = getTextColor(nodeData.level, isExpanded, isActive, context);

                // --- Determine Container Background Color ---
                Color containerColor = isActive
                    ? activeColor // Active node is always golden
                    : (isExpanded
                        ? Theme.of(context).colorScheme.secondary // Expanded (not active) is teal
                        : nodeColor); // Collapsed (not active) uses level color


                return GestureDetector(
                  onTap: () {
                    print("Tapped on: ${nodeData.label} (Level: ${nodeData.level}, ID: ${nodeData.id})");
                  ...
                  child: AnimatedContainer( // Use AnimatedContainer for smooth transitions
                    ...
                    constraints: const BoxConstraints(minWidth: 40, minHeight: 20), // Min size
                    decoration: BoxDecoration(
                      color: containerColor, // Use the calculated container color
                      borderRadius: BorderRadius.circular(20), // Consistent rounding
                      border: Border.all(
                        color: borderColor,
                        width: isActive ? 2.0 : 1.5 // <<< Thicker border for active node
                      ),
                      boxShadow: [
                        BoxShadow(
                          // Shadow color based on the *actual* container color
                          color: containerColor.withOpacity(isActive ? 0.5 : 0.3), // <<< Brighter shadow for active
                    ...
                    child: Center( // Center text
                      child: Text(
                         ...
                         style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                            ...
                            fontWeight: isActive ? FontWeight.bold : FontWeight.w500, // <<< Bold text for active
